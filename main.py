...
from app.services.database import init_database, get_connection, execute_many, clear_history, fetch_all
...
from app.ui.dialogs import HealthCheckDialog, JsonViewerDialog, BackupSummaryDialog
...
class MainWindow(QMainWindow):
    ...
    def select_pdf_destination(self):
        ...
        if directory:
            try:
                ...
                logger.warning("No se pudo crear la carpeta destino de PDFs: %s", directory, exc_info=True)
            self.txt_pdf_dest.setText(directory)
            self.settings.setValue(AppSettings.KEY_PDF_DEST, directory)
            self.settings.sync()
    ...
    def __init__(self):
        ...
        self._history_pending_apply_filters = True
        self._history_reload_delay_ms = 250
    ...
    def create_send_page(self):
        ...
        default_pdf_dir = os.path.join(os.path.expanduser("~"), "FactuNabo", "Facturas PDF")
        stored_pdf_dest = self.settings.value(AppSettings.KEY_PDF_DEST, default_pdf_dir)
        self.txt_pdf_dest = QLineEdit(str(stored_pdf_dest or default_pdf_dir))
    ...
    def create_history_page(self):
        ...
        btn_refresh.clicked.connect(lambda: self.queue_history_reload(immediate=True))
    ...
    def create_config_page(self):
        ...
        # --- Grupo 3: Herramientas de mantenimiento ---
        group3_frame = QFrame()
        group3_frame.setProperty("class", "ConfigGroup")
        apply_shadow(group3_frame, blur=20, offset_y=3)
        group3_layout = QVBoxLayout(group3_frame)
        group3_layout.setSpacing(12)

        g3_title = QLabel("Mantenimiento y soporte")
        g3_title.setFont(QFont(QApplication.font().family(), 18, QFont.Bold))
        group3_layout.addWidget(g3_title)

        tools_row = QHBoxLayout()
        tools_row.setSpacing(12)

        btn_health = AnimatedButton("🩺 Diagnóstico del sistema")
        btn_health.setToolTip("Ejecuta comprobaciones rápidas de base de datos, carpetas y navegador.")
        btn_health.setStyleSheet("padding: 8px 20px; min-height: 32px; font-size: 13px;")
        btn_health.clicked.connect(self.open_health_checks)
        tools_row.addWidget(btn_health)

        btn_backup = AnimatedButton("💾 Copia de seguridad")
        btn_backup.setToolTip("Genera un archivo ZIP con la base de datos y usuarios.")
        btn_backup.setStyleSheet("padding: 8px 20px; min-height: 32px; font-size: 13px;")
        btn_backup.clicked.connect(self.create_backup_archive)
        tools_row.addWidget(btn_backup)

        btn_api_viewer = AnimatedButton("🧾 Última respuesta API")
        btn_api_viewer.setToolTip("Visualiza el JSON bruto de la última respuesta de la API.")
        btn_api_viewer.setStyleSheet("padding: 8px 20px; min-height: 32px; font-size: 13px;")
        btn_api_viewer.clicked.connect(self.open_last_api_response)
        tools_row.addWidget(btn_api_viewer)

        btn_update_templates = AnimatedButton("⬇️ Actualizar plantillas")
        btn_update_templates.setToolTip("Comprueba si hay nuevas plantillas y permite descargarlas.")
        btn_update_templates.setStyleSheet("padding: 8px 20px; min-height: 32px; font-size: 13px;")
        btn_update_templates.clicked.connect(self.check_template_updates)
        tools_row.addWidget(btn_update_templates)

        tools_row.addStretch()
        group3_layout.addLayout(tools_row)
        layout.addWidget(group3_frame)
    ...
    def clear_history_execute(self):
        try:
            clear_history()
            self.stats_service.invalidate()
            self.show_toast("✅ Historial de envíos borrado.")
            self.load_history()
        except Exception as e:
            self.show_error(f"Error al borrar el historial: {e}")
            logger.exception("Error borrando historial")
    # --- [FIN NUEVAS FUNCIONES] ---

    def open_health_checks(self):
        pdf_dest = self.txt_pdf_dest.text().strip() if hasattr(self, "txt_pdf_dest") else ""
        results = run_health_checks(pdf_dest, getattr(self, "browser_display_name", ""))
        dlg = HealthCheckDialog(results, self)
        style_sheet_content = self._get_themed_stylesheet()
        if style_sheet_content:
            dlg.setStyleSheet(style_sheet_content)
            dlg.setProperty("theme", self.theme)
    ...
    def create_backup_archive(self):
        try:
            ...
            dlg = BackupSummaryDialog(backup_path, self)
            style_sheet_content = self._get_themed_stylesheet()
            if style_sheet_content:
                dlg.setStyleSheet(style_sheet_content)
                dlg.setProperty("theme", self.theme)
            ...
            self.show_error(f"No se pudo crear la copia de seguridad: {exc}")

    def open_last_api_response(self):
        responses_dir = Path(resource_path("responses"))
        if not responses_dir.exists():
            self.show_toast("⚠️ No se encontraron respuestas de la API.")
            return
        json_files = sorted(responses_dir.glob("api_*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
        if not json_files:
            self.show_toast("⚠️ No se encontraron respuestas de la API.")
            return
        latest = json_files[0]
        dlg = JsonViewerDialog(latest, self)
        style_sheet_content = self._get_themed_stylesheet()
        if style_sheet_content:
            dlg.setStyleSheet(style_sheet_content)
            dlg.setProperty("theme", self.theme)
    ...
    def check_template_updates(self):
        templates_dir = Path(resource_path("Plantillas Facturas"))
        schema_path = Path(resource_path("EsquemaProformas.xsd"))
        if not schema_path.exists():
            self.show_error("No se encontró 'EsquemaProformas.xsd' en los recursos.")
            return

        update_url = self.settings.value(AppSettings.KEY_TEMPLATE_URL, "")
        if not update_url:
            url, ok = QInputDialog.getText(
                self,
                "URL de actualización",
                "Introduce la URL de la plantilla o esquema actualizado:",
                text="",
            )
            if not ok or not url.strip():
                return
            update_url = url.strip()
            self.settings.setValue(AppSettings.KEY_TEMPLATE_URL, update_url)
            self.settings.sync()

        checksum = compute_file_checksum(schema_path)
        result = check_remote_template_version(update_url, checksum)
        estado = result.get("estado", "INFO")
        detalle = result.get("detalle", "")

        if estado == "OK":
            QMessageBox.information(self, "Actualización de plantillas", detalle or "Todo actualizado.")
            return

        if estado == "ERROR":
            QMessageBox.warning(self, "Actualización de plantillas", detalle or "No se pudo comprobar la actualización.")
            return

        # Hay una versión diferente disponible
        reply = QMessageBox.question(
            self,
            "Actualizar plantillas",
            "Se ha detectado una versión nueva. ¿Quieres descargarla ahora?",
        ...
        try:
            ...
            download_template(update_url, destination)
            self.show_toast("✅ Plantilla actualizada correctamente.")
        except Exception as exc:
            logger.exception("Error descargando plantilla")
            self.show_error(f"No se pudo descargar la plantilla: {exc}")
    ...
    def select_excel(self, path=None):
        ...
        try:
            ...
            logger.exception("Error al poblar la tabla de previsualización")
        # --- [FIN NUEVO] ---
    ...
    def on_downloads_done_gui(self):
        ...
        try:
            ...
            try:
                ...
                logger.exception("Error sincronizando rutas PDF tras descarga")
        ...
        for i, item in enumerate(data):
            ...
            try:
                ...
                # LA LLAMADA A self.save_to_history(item) HA SIDO ELIMINADA DE AQUÍ
                 ...
                 logger.exception("Error procesando item del summary para la UI")
    ...
    def on_finished(self, *args, **kwargs):
        ...
        try:
            ...
            logger.exception("Error en on_finished del envío")
        finally:
            if hasattr(self, "btn_send"):
                self.btn_send.setEnabled(True)
    ...
    def save_summary_to_history(self, summary_data: list):
        ...
        try:
            ...
            self.stats_service.invalidate()
            self.queue_history_reload(immediate=True)
        except Exception as e:
            logger.exception("Error guardando resumen en BBDD")
    ...
    def _update_pdf_paths_in_history(self, summary_data: list):
        ...
        try:
            ...
            logger.exception("Error actualizando rutas de PDF en historial")
    ...
    def load_history(self, apply_filters=True):
        ...
        try:
            query = (
                "SELECT id, fecha_envio, num_factura, empresa, estado, detalles, pdf_url, "
                "pdf_local_path, importe, cliente FROM envios WHERE 1=1"
            )
            params = []
            ...
            query += " ORDER BY fecha_envio DESC LIMIT 1000"
            rows = list(fetch_all(query, params))

            table.setRowCount(0)

            for db_id, fecha, num_factura, empresa, estado, detalles, pdf_url, pdf_local_path, importe, cliente in rows:
                row_index = table.rowCount()
                table.insertRow(row_index)

                table.setItem(row_index, 0, QTableWidgetItem(str(db_id)))
                table.setItem(row_index, 1, QTableWidgetItem(fecha))
                table.setItem(row_index, 2, QTableWidgetItem(num_factura))
                table.setItem(row_index, 3, QTableWidgetItem(empresa))
                table.setItem(row_index, 4, QTableWidgetItem(cliente))

                item_importe = QTableWidgetItem(format_eur(importe))
                item_importe.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter)
                table.setItem(row_index, 5, item_importe)

                chip = StatusChip(estado)
                table.setCellWidget(row_index, 6, chip)

                table.setItem(row_index, 7, QTableWidgetItem(detalles))
                ...
                if btn_pdf:
                    table.setCellWidget(row_index, 8, btn_pdf)
            ...
            if hasattr(self, 'history_filter_empresa'):
                ...
                empresas_en_bd = {empresa for (_, _, _, empresa, *_rest) in rows if empresa}
                for emp in sorted(empresas_en_bd - empresas_actuales):
                    self.history_filter_empresa.addItem(emp)

            self.update_dashboard_stats()
        except Exception as e:
            self.show_toast(f"Error cargando histórico: {str(e)}")
            logger.exception("Error cargando histórico")
        finally:
            table.setUpdatesEnabled(True)
    
    def apply_history_filters(self, *args):
        """Aplica los filtros de búsqueda al histórico."""
        self.queue_history_reload()
    ...
    def clear_history_filters(self):
        ...
        self.queue_history_reload(apply_filters=False, immediate=True)

    def queue_history_reload(self, apply_filters=True, immediate=False):
        self._history_pending_apply_filters = apply_filters
        if immediate:
            self.history_reload_timer.stop()
            self.load_history(apply_filters=apply_filters)
        else:
            self.history_reload_timer.start(self._history_reload_delay_ms)

    def _execute_history_reload(self):
        self.load_history(apply_filters=self._history_pending_apply_filters)
    ...
    def export_history(self):
        ...
        try:
            ...
            logger.exception("Error al exportar historial")
    ...
    def update_dashboard_stats(self):
        ...
        if not all([self.total_label, self.success_label, self.month_total_label, self.month_count_label]):
            logger.warning("Faltan referencias a las etiquetas del Dashboard en update_dashboard_stats")
            return
        
        try:
            stats = self.stats_service.get_dashboard_stats()
            self.total_label.setText(str(stats.total_success))
            self.success_label.setText(str(stats.total_success))
            self.month_count_label.setText(str(stats.month_count))
            self.month_total_label.setText(format_eur(stats.month_total))

        except Exception as e:
            self.show_toast(f"Error actualizando dashboard: {str(e)}")
            logger.exception("Error actualizando dashboard")
    ...
    def populate_dashboard_filters(self):
        ...
        try:
            with get_connection(readonly=True) as conn:
                cursor = conn.cursor()
                rows = cursor.execute(
                    "SELECT DISTINCT empresa FROM envios WHERE empresa IS NOT NULL AND empresa != '' ORDER BY empresa"
                ).fetchall()
            for (empresa,) in rows:
                self.dash_combo_empresas.addItem(str(empresa), userData=str(empresa))
        except Exception:
            logger.exception("Error cargando empresas para el dashboard")
    ...
    def run_dashboard_query(self):
        ...
        try:
            ...
            logger.exception("Error ejecutando consulta del dashboard")
            return
    ...
    def _update_send_badge(self):
        ...
        try:
            ...
            logger.exception("Error actualizando badge de envío")
...
def main():
    ...
    try:
        ...
        logger.warning("No se encontró 'dialog_shim.py' o 'modern_dialogs.py'. Se usarán diálogos nativos.")
    # --- [FIN NUEVO] ---
